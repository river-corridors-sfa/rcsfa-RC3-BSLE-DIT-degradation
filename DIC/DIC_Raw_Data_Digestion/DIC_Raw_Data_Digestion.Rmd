...
rsquared1 <- rsquared((unlist(dic_crm%>%filter(areaID=="area2")%>%select("Area (net avg)"))),c(1.2,1.5,1.8), slope2,intercept2)
time1 <- dic_crm5[3,3]
...
rsquared2 <- rsquared((unlist(dic_crm%>%filter(areaID=="area3")%>%select("Area (net avg)"))),c(1.2,1.5,1.8), slope3,intercept3)
time2 <- dic_crm5[6,3]

#find the difference in the points of interest for later use
dslope <- slope2-slope3
dintercept <- intercept2-intercept3
dtime <- as.numeric(difftime(time3,time2, units="mins"))
```
