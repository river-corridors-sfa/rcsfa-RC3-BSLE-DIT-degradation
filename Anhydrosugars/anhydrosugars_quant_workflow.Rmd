...
levo_raw <- read_csv("./Example_data/QuantCSV.csv", skip =1, skip_empty_rows = TRUE) %>%
  rename(header = `200ppb.d`,
         area = `...2`,
         instrument = QQQ) %>%
  mutate(row_number = row_number())  # Add a row number for reference

levo_raw_areas <- levo_raw %>%
  mutate(is_levoglucosan = ifelse(str_detect(header, "Levoglucosan"), TRUE, FALSE)) %>%
